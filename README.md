...
- This custom OSGi module injects in a new template variable called 'ctCollectionName' for use in Workflow Emails where Publications is used.
  - ctCollectionId is available, the custom code uses ctCollectionId and _ctCollectionLocalServiceLocalService.fetchCTCollection(ctCollectionId) to get the Publication record and it's name.
  ...
  - The variable can be used in the Notification Description field (i.e. the Email Subject) or in the Notification Template field (i.e. the Email Body) as follows:
...
- The custom OSGi component TemplateNotificationMessageGenerator is used in place of the OOTB OSGi component TemplateNotificationMessageGenerator. The following OOTB OSGi component must be Blacklisted:
